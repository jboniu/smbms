...
$(function(){
	...
	id.on("blur",function(){
		if(id.val() != null && id.val() != ""){
			...
			validateTip(id.next(),{"color":"red"},imgNo+" id不能为空，请重新输入",false);
		}
	}).on("focus",function(){
		//显示友情提示
		validateTip(id.next(),{"color":"#666666"},"* 请输入订单编码",false);
	}).focus();
	...
	addBtn.on("click",function(){
		...
		else if(billCode.attr("validateStatus") != "true"){
			billCode.blur();
